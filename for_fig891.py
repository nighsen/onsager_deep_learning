...
def LISTA():
    ...
    layers_tied = networks.build_LISTA(prob,T=6,initial_lambda=.1,untied=False)
    #layers_untied = networks.build_LISTA(prob,T=6,initial_lambda=.1,untied=True)
    # plan the learning
...
def LAMP():
    ...
    layers_tied = networks.build_LAMP(prob,T=6,shrink='bg',untied=False)
    layers_untied = networks.build_LAMP(prob,T=6,shrink='bg',untied=True)
    # plan the learning
    training_stages_tied = train.setup_training(layers_tied,prob,trinit=1e-3,refinements=(.5,.1,.01) )
    training_stages_untied = train.setup_training(layers_untied,prob,trinit=1e-3,refinements=(.5,.1,.01) )
...
def LAMP_Shrinkage(shrink_name, untied_flag, kappa_num=None):
    ...
    # build a LAMP network to solve the problem and get the intermediate results so we can greedily extend and then refine(fine-tune)
    layers = networks.build_LAMP(prob,T=6,shrink=shrink_name,untied=untied_flag)
...
def LVAMP_Shrinkage(shrink_name, kappa_num=None):
    ...
    layers = networks.build_LVAMP(prob,T=6,shrink=shrink_name)
